...
class BipartiteStmt(Visitable):
	def __init__(self, fab_params, fab_formula, disc_params, disc_formula):
		""" Construct a new RequirementsStmt - a formula (an 'and' literal where each sub-literal's terms are vars,
		or a single literal whose terms are vars.
...
def parse_decomp_stmt(it):
	...
	it = it.next()
	if not it.try_match(':sub-params') and not it.try_match(':fabula-params') and not it.try_match(':discourse-params'):
		raise ValueError("Error decomp statement needs :sub-params, :discourse-params, or :fabula-params")
	varList = parse_typed_var_list(next(it))

	if not it.try_match(':requirements') and not it.try_match(':fabula-requirements') and not it.try_match(':discourse-requirements'):
	...
	if look is not None:
		discourse_name = next(it)
		fab_formula = parse_formula(ni)
		if discourse_name.contents == ':discourse-params':
			# it = it.next()
			disc_vars = parse_typed_var_list(next(it))
			if not it.try_match(':discourse-requirements'):
				raise ValueError('Error bipartite statement needs :discourse-requirements')
			disc_formula_ni = next(it)
			if disc_formula_ni.empty():
				raise ValueError('Discourse params provided but no discourse requirements')
			return BipartiteStmt(varList, fab_formula, disc_vars, parse_formula(disc_formula_ni))
		else:
			raise ValueError('too many decomp sections')
